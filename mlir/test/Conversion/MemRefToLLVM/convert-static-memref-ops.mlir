...
// CHECK: %[[bump:.*]] = llvm.sub %[[alignment]], %[[st1]] : i64
...
// CHECK: %[[st1:.*]] = llvm.mlir.constant(1 : index) : i64
// The zero size, stride and element count all share one constant.
// CHECK: %[[allocated:.*]] = llvm.alloca %[[sz2]] x f32 : (i64) -> !llvm.ptr
...
func.func @memref.reshape(%arg0: memref<4x5x6xf32>) -> memref<2x6x20xf32> {
  ...
  // The descriptor constants are all materialized at the top of the function.
  // CHECK: %[[one:.*]] = llvm.mlir.constant(1 : index) : i64
  ...
  // CHECK: %[[insert6:.*]] = llvm.insertvalue %[[twenty0]], %[[insert5]][4, 1] : !llvm.struct<(ptr, ptr, i64, array<3 x i64>, array<3 x i64>)>
...
func.func @memref.reshape.dynamic.dim(%arg: memref<?x?x?xf32>, %shape: memref<4xi64>) -> memref<?x?x12x32xf32> {
  ...
  // CHECK-DAG: %[[zero0:.*]] = llvm.mlir.constant(0 : index) : i64
  // CHECK-DAG: %[[one0:.*]] = llvm.mlir.constant(1 : index) : i64
  // CHECK-DAG: %[[thirty_two0:.*]] = llvm.mlir.constant(32 : index) : i64
  // CHECK-DAG: %[[twelve:.*]] = llvm.mlir.constant(12 : index) : i64
  // CHECK-DAG: %[[three_hundred_and_eighty_four:.*]] = llvm.mlir.constant(384 : index) : i64
  ...
  // CHECK: %[[insert6:.*]] = llvm.insertvalue %[[thirty_two0]], %[[insert5]][4, 2] : !llvm.struct<(ptr, ptr, i64, array<4 x i64>, array<4 x i64>)>

  // CHECK: %[[shape_ptr0:.*]] = llvm.extractvalue %[[shape_cast]][1] : !llvm.struct<(ptr, ptr, i64, array<1 x i64>, array<1 x i64>)>
  // CHECK: %[[shape_gep0:.*]] = llvm.getelementptr inbounds|nuw %[[shape_ptr0]][%[[one0]]] : (!llvm.ptr, i64) -> !llvm.ptr, i64
  ...
  // CHECK: %[[shape_gep1:.*]] = llvm.getelementptr inbounds|nuw %[[shape_ptr1]][%[[zero0]]] : (!llvm.ptr, i64) -> !llvm.ptr, i64
...
func.func @memref.reshape_index(%arg0: memref<?x?xi32>, %shape: memref<1xindex>) ->  memref<?xi32> {
  ...
  // CHECK: %[[shape_ptr0:.*]] = llvm.extractvalue %[[shape_cast:.*]][1] : !llvm.struct<(ptr, ptr, i64, array<1 x i64>, array<1 x i64>)>
  // CHECK: %[[shape_gep0:.*]] = llvm.getelementptr inbounds|nuw %[[shape_ptr0:.*]][%[[zero0]]] : (!llvm.ptr, i64) -> !llvm.ptr, i64
  // CHECK: %[[shape_load0:.*]] = llvm.load %[[shape_gep0:.*]] : !llvm.ptr -> i64
