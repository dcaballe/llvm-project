...
static llvm::cl::opt<bool> clMemRefFilter(
    PASS_NAME "-memref-filter",
    llvm::cl::desc(
...
std::unique_ptr<OpPassBase<FuncOp>> mlir::createTestAffineDataCopyPass() {
  return std::make_unique<TestAffineDataCopy>();
}
...
void TestAffineDataCopy::runOnFunction() {
  ...
  if (clMemRefFilter) {
    // Gather MemRef filter. For simplicity, we use the first memref found in
    // the innermost loop.
...
static PassRegistration<TestAffineDataCopy>
    pass(PASS_NAME, "Tests affine data copy utility functions.");
