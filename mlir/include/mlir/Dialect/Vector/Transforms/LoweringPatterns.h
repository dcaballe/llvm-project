...
/// stores when beneficial. If `allowMultipleUses` is false, the patterns are
/// only applied to vector transfer reads with a single use. Otherwise, only
/// vector transfer reads with a single use will be lowered.
