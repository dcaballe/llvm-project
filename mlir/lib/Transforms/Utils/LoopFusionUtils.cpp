...
FusionResult mlir::canFuseLoops(AffineForOp srcForOp, AffineForOp dstForOp,
  ...
  // strategies.
  if (fusionStrategy.value == FusionStrategy::ProducerConsumer) {
  ...
  // assumptions expected by the fusion strategy.
  SmallVector<Operation *, 4> strategyOpsA;
  switch (fusionStrategy.value) {
  case FusionStrategy::None:
  ...
  case FusionStrategy::Sibling:
    // Sibling fusion (AffineLoopFusion pass) only takes into account loads
    // in 'srcForOp' to compute the slice union.
    for (Operation *op : opsA) {
