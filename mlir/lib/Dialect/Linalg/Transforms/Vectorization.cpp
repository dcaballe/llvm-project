...
struct VectorizationState {
  ...
  Operation *maskOperation(RewriterBase &rewriter, Operation *opToMask,
                           LinalgOp linalgOp,
                           Optional<AffineMap> maybeMaskingMap = llvm::None);
...
Value VectorizationState::getOrCreateMaskFor(
  ...
  // already created.
  if (activeMaskCache.count(maskingMap)) {
    Value mask = activeMaskCache[maskingMap];
  ...
  // Create the mask based on the value uppermension size values.
...
VectorizationState::maskOperation(RewriterBase &rewriter, Operation *opToMask,
  ...
  Value mask = getOrCreateMaskFor(rewriter, opToMask, linalgOp, maybeMaskingMap);
...
vectorizeAsLinalgGeneric(RewriterBase &rewriter, VectorizationState &state,
  ...
  for (OpOperand *opOperand : linalgOp.getOpOperandsMatchingBBargs()) {
    ...
    AffineMap maskMap = indexingMap.dropResults(zeroPos);
    ...
    read = state.maskOperation(rewriter, read, linalgOp, maskMap);
