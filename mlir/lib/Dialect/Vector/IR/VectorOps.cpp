...
ParseResult MaskOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  // Parse all the types and classify them.
  ...
  // Resolve operands.
  if (parser.resolveOperand(mask, types[0], result.operands))
    return failure();

  if (parsePassthru.succeeded())
    if (parser.resolveOperand(passthru, types[1], result.operands))
      return failure();
...
void mlir::vector::MaskOp::print(OpAsmPrinter &p) {
  ...
  if (singleBlock && singleBlock->getOperations().size() > 1)
    p.printOperationWithoutResults(&singleBlock->front());
  p << " }";
...
LogicalResult MaskOp::verify() {
  ...
  if (passthru) {
    if (!maskableOp.supportsPassThru())
      return emitOpError(
          "doesn't expect a pass-thru argument for this maskable operation");

    if (maskableOp->getNumResults() != 1)
       return emitOpError(
          "expects result when pass-thru argument is provided");

    if (passthru.getType() == maskableOp->getResultTypes()[0])
      return emitOpError(
          "expects pass-thru type to match result type");
  }
