...
/// Returns true if the transfer read is a masked scalar load in the canonical
/// form generated by `RewriteScalarExtractOfTransferReadBase` .
...
class RewriteScalarExtractElementOfTransferRead
  ...
  void rewrite(vector::ExtractElementOp extractOp,
    ...
    if (isa<MemRefType>(xferOp.getSource().getType())) {
      ...
      // Masked case: generate a 1-D vector load with an all-zero mask except
      // for one bit that is extracted from the original mask. The vector size
      // is the size of the original vector's trailing dimension.
      auto oldMask = xferOp.getMask();
      auto singleMaskBit = rewriter.create<vector::ExtractOp>(
          loc, oldMask, extractOp.getPosition());

      int64_t singleDimSize = xferOp.getVectorType().getShape().back();
